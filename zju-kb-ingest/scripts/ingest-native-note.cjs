...
async function main() {
  ...
  const existingDocs = (await ownNotes(`00 LOG 01｜资料登记｜${person.name}`, credentials)).filter((x) => titleMatches(x.title, title));
  if (existingDocs.length > 1) fail('native-note-recovery-ambiguous');
  const libraries = await post('wiki', 'get_addable_knowledge_base_list', { cursor: '', limit: 50 }, credentials); const library = (libraries.addable_knowledge_base_list || []).find((x) => x?.name === KB_NAME); if (!library) fail('target-library-not-addable');
  ...
  if (!docId) {
    ...
    for (let attempt = 0; attempt < 3 && !docId; attempt += 1) {
      await wait(2500);
      let docs;
      try { docs = await ownNotes(`00 LOG 01｜资料登记｜${person.name}`, credentials); } catch { fail('NATIVE_NOTE_CREATED_PENDING_ASSOCIATION-retry-same-command-do-not-create-again'); }
  ...
  const logTitle = `00 LOG 01｜资料登记｜${person.name}`; const log = (await ownNotes(logTitle, credentials)).find((x) => x.title === logTitle); if (!log) fail('own-log-01-not-found');
